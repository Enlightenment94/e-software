<?php
function updateRecord($by, $afo, $author, $otxt, $tags){
	$conn = new mysqli($GLOBALS['servername'], $GLOBALS['username'], $GLOBALS['password'], $GLOBALS['dbname']);
    
	if ($conn->connect_error) {
		die("Connection failed0: " . $conn->connect_error);
	}else {
        $sql = "UPDATE " .  $GLOBALS['table'] . " SET afo='" . $afo . "', author='" . $author . "', otxt='" . $otxt . "' WHERE afo_id='" . $by . "'"; 
        echo $sql;
		...
		$sql = "DELETE FROM " . $GLOBALS['beforeIds'] . $GLOBALS['table'] . " WHERE afo_id='" . $by . "'";
        echo $sql . "</br>";
        $resQuery = $conn->query($sql);           
        if ($resQuery === TRUE){              
        }else {
        ...
        $n = count($tagIdArr);
        for($i = 0; $i < $n; $i++){
		    $sqlInsertIdsTags = "insert into " . $GLOBALS['beforeIds'] . $GLOBALS['table'] . " (tag_id, afo_id) values ('" . $tagIdArr[$i] .  "','" . $by . "')";
